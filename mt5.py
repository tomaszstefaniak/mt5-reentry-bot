...
# — Use MetaTrader5 on Windows, otherwise use stub —
...
class LimitOrder:
    ...
    adjust_wait: float = 0.0      # seconds for automatic
    adjust_pct: float = 0.0       # percent for automatic
    pip_distance: float = 0.0     # pips for manual
    active: bool = True
...
class MT5TradingBot:
    ...
    def _manual(self, lo: LimitOrder):
        ...
        delta = lo.pip_distance * point * (-1 if lo.direction == 'LONG' else 1)
    ...
    def _send(self, price: float, lo: LimitOrder):
        request = {
            ...
            'symbol': lo.symbol,
            'volume': lo.volume,
            'type': mt5.ORDER_TYPE_BUY_LIMIT if lo.direction == 'LONG' else mt5.ORDER_TYPE_SELL_LIMIT,
            'price': price,
            'sl': lo.sl,
            'tp': lo.tp,
            'deviation': 10,
